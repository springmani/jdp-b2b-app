	...
	public function query(
		...
		if ( is_wp_error( $response ) ) {
			...
			throw new ClientQueryException(
				sprintf(
					'WordPress request error: %s. Please contact our support team.',
					wp_kses_data( $response->get_error_message() )
				)
			);
		}
		...
		if ( 400 === $response_http_code ) {
			...
			throw new ClientQueryException( 'Please upgrade WP Engine Smart Search plugin to the latest version.', 400 );
		}
		...
		if ( 404 === $response_http_code ) {
			...
			throw new ClientQueryException( 'Please verify your WP Engine Smart Search URL.', 404 );
		}
		...
		if ( $response_http_code >= 500 ) {
			...
			throw new ClientQueryException( 'An unexpected server error occurred. Please contact support.', 500 );
		}
		...
		if (
			...
			if ( 'Authentication failed' === $data || 'Authentication header not present or malformed' === $data ) {
				...
				throw new ClientQueryException( 'Please verify your WP Engine Smart Search access token.', 401 );
			}
		...
		if ( empty( $data ) || ( empty( $response_data ) && ! is_array( $response_data ) ) ) {
			$logger->log(
				sprintf(
					'BAD_JSON: Response was not in JSON format, DATA: %s RESPONSE CODE: %s',
					$data,
					$response_http_code
				)
			);
		...
		if ( ! empty( $response_data['errors'] ) ) {
			...
			throw new ClientQueryGraphqlErrorsException( wp_kses_data( $graphql_errors[0]['message'] ) );
		}
