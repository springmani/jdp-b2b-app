...
class Sync_Lock_Manager {
	...
	public function get_status() {
		$deserialized = \AtlasSearch\Support\WordPress\get_option(
			Sync_Lock_Status::OPTIONS_WPE_ATLAS_SEARCH_SYNC_STATUS,
			new Sync_Lock_Status()
		);
	...
	public function can_start( DateTime $moment, ?string $uuid = null ): bool {
		...
		$expiry_threshold = $last_updated_copy->add(
			new \DateInterval( "PT{$this->get_rolling_timeout_expiry_seconds()}S" )
		);

		return $moment >= $expiry_threshold;
	}
