...
abstract class Configurable {
	...
	public function get_acf_search_config( string $model_name, array $existing_config, array $current_config ): array {
		$acf_field_groups = \acf_get_field_groups( [ 'post_type' => $model_name ] );
		$result           = [];
		...
		foreach ( $acf_field_groups as $acf_field_group ) {
			...
			foreach ( $acf_fields as $acf_field ) {
				...
				$name              = String_Transformation::camel_case( $acf_field['name'], [ '_' ] );
		...
		$current_model_config          = $current_config[ $model_name ] ?? [];
