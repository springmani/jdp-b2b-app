...
class Search_Config {
	...
	public function get_fields(): array {
		return get_option( self::WPE_CONTENT_ENGINE_SEARCH_FIELDS, [] );
	}
	...
	public function generate_search_config( array $existing_config ): array {
		...
		return [
			'models'             =>
				$posts->get_config( $existing_config['models'] ?? [] ),
			'fuzzy'              => [
				'enabled'  => $existing_config['fuzzy']['enabled'] ?? false,
				'distance' => $existing_config['fuzzy']['distance'] ?? 1,
			],
			'recency'            => [
				'enabled' => $existing_config['recency']['enabled'] ?? false,
				'days'    => $existing_config['recency']['days'] ?? 7,
			],
			'disabledModelNames' => $existing_config['disabledModelNames'] ?? [],
