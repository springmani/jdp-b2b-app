...
class WPE_Atlas_Search_Settings_Page {
	public const SETTINGS_SCRIPT = 'wpengine-smart-search-js';
	...
	public const RECOMMENDATIONS_PAGE = 'wpengine-smart-search&amp;view=recommendations';
	...
	public function enqueue_settings_page() {
		...
		add_action(
			...
			static function () use ( $plugin_name, $capabilities ) {
				...
				wp_localize_script(
					...
					[
						...
						'haveDoneIndex'      => get_option(
							'wpe_content_engine_re_sync_has_occurred',
							false
						) ? 'true' : 'false',
				...
				if ( 'dev' === getenv( 'WPE_SMART_SEARCH_ENV' ) ) {
					wp_enqueue_script(
						self::SETTINGS_SCRIPT,
						'http://localhost:3003/wp-content/plugins/wpengine-smart-search/includes/smart-search-settings/build/static/js/bundle.js',
						'',
						wp_rand( 10, 1000 ),
						true
					);
	...
	public function add_settings_menu() {
		$icon        = include_once __DIR__ . '/views/search-menu-icon.php';
		$plugin_name = $this->resolve_plugin_name();
		...
		$has_recommendations_capability = \Wpe_Content_Engine\Helper\CapabilitiesService::has_capability(
			'RECOMMENDATIONS'
		);

		if ( ! $has_recommendations_capability || ! $this->is_network_admin_analytics_enabled() ) {
			return;
		}
	...
	public function add_network_admin_settings_menu() {
		...
		add_submenu_page( self::PAGE, 'Index Data', 'Index Data', 'manage_options', self::PAGE, '__return_null', 1 );

		add_submenu_page( self::PAGE, 'Settings', 'Settings', 'manage_options', self::SETTINGS_PAGE, '__return_null', 2 );
	...
	/**
	 * Returns true when the network-admin analytics master switch is on.
	 * Always true on single-site (no network concept exists).
	 */
	private function is_network_admin_analytics_enabled(): bool {
		if ( ! is_multisite() ) {
			return true;
		}

		$option_value = get_network_option(
			null,
			\WPE\AITK\Modules\Smart_Search\Recommendations\Recommendations_Network_Rest_Controller::NETWORK_OPTION,
			false
		);
		...
		$config = \WPE\AITK\Modules\Smart_Search\Recommendations\Data\Recommendations_Config_Data::from_array(
			$option_value
		);

		return $config->is_analytics_enabled();
	}

	private function resolve_plugin_name(): string {
		if ( is_array( $this->caps ) && in_array( 'EXTENDED_FEATURES', $this->caps, true ) ) {
			return \WPE\AITK\WP\Plugin\Plugin_Name_Resolver::ON_PLATFORM_NAME;
		}

		return \WPE\AITK\WP\Plugin\Plugin_Name_Resolver::OFF_PLATFORM_NAME;
	}
}
