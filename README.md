...
Explicit ?region= in the URL or AJAX payload (user choice / shared link)
Geo-detected continent → product_region slug
Fallback: na (North America) when geo is unknown, unmapped, or the matching term does not exist
Important behaviors:
...
Supported params: continent, country, region, city, postalcode. See WP Engine GeoTarget docs.
...
The GeoTarget continent env var is empty, and
WP_DEBUG is true or wp_get_environment_type() is local / development, and
?geoip is not present in the URL
...
GeoTarget product extension enabled (included on Premium plans).
WP Engine GeoTarget plugin installed and active.
GeoTarget profile enabled by WP Engine Support — use a continent-level cache bucket.
All three product_region term slugs (na, eu, oc) present in WP Admin.
...
Singular industries, segments, solution, or product
Post has at least one assigned region and the visitor’s detected slug is not in that set
Region Modal tab has heading, choices heading, or a sub heading for the current post type
Product Finder page is selected in Theme Settings
